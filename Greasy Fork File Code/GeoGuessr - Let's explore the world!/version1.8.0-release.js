...
/* global google, L, GM_getValue, GM_setValue, GM_xmlhttpRequest, unsafeWindow */
...
(function() {
    ...
    // ==================== EXTRACTION CACHE ====================
    // Prevents redundant Fiber walks and DOM queries within the same
    // polling cycle.  The cache is invalidated every time the XHR
    // interceptor captures new coordinates (guaranteed fresh data)
    // or when the TTL expires.

    const extractionCache = {
        result: null,       // { lat, lng } | null
        source: null,       // string identifier of winning extractor
        timestamp: 0,       // Date.now() of last successful extraction
        ttl: 400,           // ms — slightly shorter than polling interval (500ms)
        hits: 0,            // diagnostic counter
        misses: 0,          // diagnostic counter

        get() {
            if (this.result && (Date.now() - this.timestamp) < this.ttl) {
                this.hits++;
                return this.result;
            }
            return null;
    ...
    // XHR Interception for coordinate extraction
    // PATCH: guard against duplicate listener attachment per XHR instance
                ...
                try {
                    ...
                    if (matches && matches.length > 0) {
                        ...
                        if (isValidCoord(lat, lng)) {
                            ...
                            extractionCache.invalidate(); // fresh data available
                            log('XHR intercepted:', lat.toFixed(6), lng.toFixed(6));
                        }
                    }
                } catch (e) {
    ...
    function extractCoordinates() {
        // Check cache first — avoids redundant Fiber walks within the same cycle
        const cached = extractionCache.get();
        if (cached) return cached;

        try {
            // ── Method 0: XHR Interception (fastest, event-driven) ──
            ...
            if (state.platform === 'geoguessr') {
                ...
                if (fromSV) { extractionCache.set(fromSV, 'sv-canvas-geo'); return fromSV; }
            }
                ...
                try {
                    ...
                    if (location) {
                        ...
                        if (parts.length >= 2) {
                            ...
                            if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'iframe-location'); return r; }
                        }
                    }
                    ...
                    if (iLat && iLng) {
                        ...
                        if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'iframe-latlng'); return r; }
                    }
                    ...
                    if (cbll) {
                        ...
                        if (parts.length >= 2) {
                            const lat = parseFloat(parts[0]);
                            const lng = parseFloat(parts[1]);
                            if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'iframe-cbll'); return r; }
                        }
                    }
                    ...
                    if (viewpoint) {
                        ...
                        if (parts.length >= 2) {
                            const lat = parseFloat(parts[0]);
                            const lng = parseFloat(parts[1]);
                            if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'iframe-viewpoint'); return r; }
                        }
                    }
                } catch (e) {
                    continue;
            ...
            for (const selector of freeGuessrSelectors) {
                ...
                if (Array.isArray(latLong) && latLong.length === 2) {
                    const lat = latLong[0];
                    const lng = latLong[1];
                    if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'fiber-latLong'); return r; }
                }
                ...
                if (coordinates) {
                    ...
                    if (isValidCoord(lat, lng)) { const r={lat,lng}; extractionCache.set(r,'fiber-coordinates'); return r; }
                }
            }
            ...
            if (state.platform !== 'geoguessr') {
                const fromSV = extractFromGoogleSV();
                if (fromSV) { extractionCache.set(fromSV, 'sv-canvas'); return fromSV; }
            }
            ...
            try {
                ...
                if (safeWin.__gameState?.coords) {
                    ...
                    if (isValidCoord(c.lat, c.lng)) { const r={lat:c.lat,lng:c.lng}; extractionCache.set(r,'global-gameState'); return r; }
                }
                if (safeWin.gameCoordinates) {
                    const c = safeWin.gameCoordinates;
                    if (isValidCoord(c.lat, c.lng)) { const r={lat:c.lat,lng:c.lng}; extractionCache.set(r,'global-gameCoordinates'); return r; }
                }
            } catch (e) {
    ...
    const ADDRESS_QUEUE_MAX = 5; // prevent unbounded growth on network failure
    ...
    async function lookupAddress(lat, lng) {
        return new Promise((resolve, reject) => {
            ...
            // Cap queue — drop oldest if full (they are stale anyway)
    ...
    // To offset 0-4 METERS we need 0-0.000036 degrees (4 / 111000)
    function applySafeMode(coords) {
        if (!state.features || !state.features.safeMode) {
            return coords;
        }
        
        const MAX_OFFSET_METERS = 4;
        const METERS_PER_DEGREE = 111000;
        const offsetDeg = MAX_OFFSET_METERS / METERS_PER_DEGREE; // ~0.000036

        const latOffset = (Math.random() * offsetDeg * 2) - offsetDeg; // range: -offsetDeg to +offsetDeg
        const lngOffset = (Math.random() * offsetDeg * 2) - offsetDeg;
    ...
    // Auto place marker (called from monitoring when coordinates change)
    function autoPlaceMarker() {
        ...
        // Use cached state.coords from monitoring — no need to re-extract
        if (!isValidCoord(state.coords.lat, state.coords.lng)) return;
        ...
        // Remove old marker first if exists (clean placement)
        if (state.marker) {
            try {
                if (typeof google !== 'undefined' && google.maps && state.marker.setMap) {
                    state.marker.setMap(null);
        ...
        // Place marker using state.coords
        const success = toggleMarker({ lat: state.coords.lat, lng: state.coords.lng });
        if (success) {
    ...
    function createPanel() {
        ...
        document.getElementById('geohelper-save').onclick = () => {
            ...
            invalidateHotkeyCache(); // force re-read on next keypress
    ...
    // Hotkey cache — refreshed on save, avoids GM_getValue on every keypress
    let hotkeyCache = null;
    let hotkeyCacheTime = 0;
    const HOTKEY_CACHE_TTL = 10000; // 10s — re-read from storage periodically

    function getHotkeys() {
        const now = Date.now();
        if (!hotkeyCache || (now - hotkeyCacheTime) > HOTKEY_CACHE_TTL) {
            hotkeyCache = safeGM_getValue(CONFIG.HOTKEYS_STORAGE_KEY, CONFIG.DEFAULT_HOTKEYS);
            hotkeyCacheTime = now;
        }
        return hotkeyCache;
    }
                ...
                if (changed) {
                    ...
                    if (lastValidCoords) {
                        ...
                        if (distance > 0.1) { // Significant change = new round
                            ...
                            state.markerPlacedThisRound = false;
                            extractionCache.invalidate(); // force fresh extraction
